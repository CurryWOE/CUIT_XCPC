#! https://zhuanlan.zhihu.com/p/2016493797509472989
# 凸包优化
...
时间复杂度 $O(n)$，对于同时能用四边形不等式优化中的分治和二分+双端队列，和斜率优化的题，斜率优化比分治快52%，比二分+双端队列快77%
...
namespace ConvexHullTrickAndSlopeTrick //凸包优化+斜率优化
{
    ...
    //dx:X(j)单调性,1表示单增,0表示单减
    //dm:求解的是max还是min,1表示max(上凸壳),0表示min(下凸壳)
    constexpr bool dx = 1,dm = 0;
    T dp[N];
    struct P { T x, y; } Q[N];
    ...
    T Y(int j){return /*----*/;}
    T A(int i){return /*----*/;}
    T calResult(int l,int r){return Q[l].y-K(r)*Q[l].x;}
    inline bool checkFront(T le,T ri)
    {
        if constexpr(dm)
            return ri>=le;
        else
    ...
    inline bool checkBack(P& p1,P& p2,P& p3)
    { // 队尾点和新加入点的向量跟原本凸包旋转方向相反，则是上/下凸点
        // 向量p1p2与p2p3的叉积
        auto res=((__int128_t)(p2.x-p1.x)*(__int128_t)(p3.y-p1.y)-(__int128_t)(p3.x-p1.x)*(__int128_t)(p2.y-p1.y));
        if constexpr(dm!=dx) // 凸壳旋转的方向，1表示逆时针，0表示顺时针
            return res <= 0;
        else // 顺时针
            return res >= 0;
    }
    T run(int n)
    {
        ...
        for (int i = 2; i <= n; ++i)
        {
            ...
            for(T tmp;best<tail && checkFront(res,tmp=calResult(best+1,i));++best)
                res=tmp;
            dp[i] = res-A(i);
            P p(X(i), Y(i));
            while (best<tail && checkBack(Q[tail-1],Q[tail],p))
                --tail;
            Q[++tail]=p;
        }
        return dp[n];
