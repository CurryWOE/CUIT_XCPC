#! https://zhuanlan.zhihu.com/p/2016492906916107311
# 四边形不等式优化
...
void solve(int ql, int qr, int optl, int optr)
{
    ...
    for (int i = optl; i <= min(mid,optr); ++i)
        if (int val = w(i,mid); val < ans[mid])
        {
            ans[mid] = val;
            best = i;
        }
...
/*----------f_3--------------*/
ll ans[2][N];
void solve(int ql, int qr, int optl, int optr,ll* las,ll* nxt)
{
    int mid = (ql + qr) >> 1, best=optl;
    for (int i = optl; i<=min(mid-1,optr); ++i)
        if(ll val=las[i]+calc(i+1,mid); val < nxt[mid])
        {
            nxt[mid] = val;
            best = i;
        }
    if(ql<mid)
        solve(ql, mid - 1, optl, best,las,nxt);
    if(qr>mid)
        solve(mid + 1, qr, best, optr,las,nxt);
}
ll solution()
{
    memset(ans[0],0x3f,sizeof(ans[0][0])*(n+1));
    for(int i=1;i<=n;++i)
        ans[1][i]=calc(1,i);
    for(int i=1;i<m;++i)
        solve(1,n,1,n,ans[i&1],ans[(i&1)^1]);
    return ans[m&1][n];
}
/*----------莫队,计算答案的信息作为全局变量--------------*/
int le=1,ri=0;
...
```
复杂度为 $O(n\log n)$，对于同时能使用分治或二分+双端队列的问题，分治快4%

如果是 $f_3$，且n=1e5,m=1e2，分治能比二分+双端队列，快58%
### 层级Knuth
可以解决下列条件都满足的问题：
...
对于同时能使用分治或二分+双端队列或此做法的问题，且n=4e3,m=8e2，此做法比分治快26%，比二分+双端队列快29%
```cpp
int ans[2][N],opt[2][N];
    ...
    for (int j = 1, t=1; j <= m; ++j, t^=1)
        for (int i = n; i >= 1; --i)// 位置从后往前，保证 opt[j][i+1] 已知
            for (int k = opt[t^1][i]; k <= min(i,opt[t][i + 1]); ++k)
                if(int cur = ans[t^1][k] + w(k+1, i); cur < ans[t][i])
                {
                    ans[t][i] = cur;
                    opt[t][i] = k;
                }
...
void solve(int n)
{
    ...
    for(int i=1;i<=n;++i)
        opt[i]=i;
    //这个写法比第一层循环是长度的写法，对于莫队计算w(l,r)，少移动一半步数
    for(int l=n-1;l;--l)
        for(int r=l+1;r<=n;++r)
            for(int k=opt[l][r-1],cost=w(l,r);k<=opt[l+1][r];++k)
                if(int res=ans[l][k]+ans[k+1][r]+cost; res<ans[l][r])
                {
                    ans[l][r]=res;
                    opt[l][r]=k;
                }
