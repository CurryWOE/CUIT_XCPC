...
为了快速求LCA，同深度节点表示的区间有相同长度，且静态开点，节点 l 和 r 的LCA的深度，等于 l^r 的最高位
```cpp
...
namespace CatTree
{
    ...
    inline void init(uint32_t n)
    {
        ...
        for(uint32_t dep=1,len=2;len<n;++dep,len<<=1)
            for(uint32_t l=0;l<n;l+=(len<<1))
            {
                uint32_t mid=l+len;
                for(uint32_t i = mid-1; i > l; --i)
                    ct[dep][i-1] = max(ct[dep][i], ct[dep][i-1]);
                for(uint32_t i = mid ,r=min(mid+len, n)-1; i < r; ++i)
                    ct[dep][i+1] = max(ct[dep][i], ct[dep][i+1]);
            }
    }
    inline int query(uint32_t l,uint32_t r)
    {
        if(l==r)
            return a[l];
        uint32_t dep=bit_width(l^r)-1;
        return min(ct[dep][l],ct[dep][r]);
    }
}
```
## 分块猫树
...
{
    uint32_t a[N],prefixInBlock[N],suffixInBlock[N],blockMax[N/K],ct[bit_width(N/K)][N/K];
    inline void init(uint32_t n)
    {
        ...
        for(uint32_t i=0;i<block;++i)
            blockMax[i]=suffixInBlock[i*K];
        for(uint32_t i=0;i<bit_width(block);++i)
            memcpy(ct[i],blockMax,sizeof(uint32_t)*block);
    ...
    inline uint32_t query(uint32_t l, uint32_t r)
    {
        ...
        uint32_t res = max(suffixInBlock[l], prefixInBlock[r]);
        if(bl+1==br)
            return res;
        ++bl,--br;
        if(bl==br)
            return max(res,blockMax[bl]);
        uint32_t dep=bit_width(bl^br)-1;
        return max({res,ct[dep][bl],ct[dep][br]});
    }
}
```
