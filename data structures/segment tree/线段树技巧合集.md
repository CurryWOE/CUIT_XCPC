...
namespace LiChaoTree
{
    ...
    inline bool better(int lineA,int lineB,X x){}//return true if lineA is better than lineB in X=b
    inline int getBetter(int lineA,int lineB,X x){}//return better line
    void __InsertLine(int id,X l,X r,int LineID)
    {
        int &u=lict[id];
        X mid = (l+r)>>1;
        if (better(LineID,u,mid))
            swap(u, LineID);
        if (better(LineID,u,l))
            __InsertLine(id<<1, l, mid, LineID);
        else if(better(LineID,u,r))
            __InsertLine(id<<1|1, mid + 1, r, LineID);
    }
    ...
    void addLine(X x0,X y0,X x1,X y1)
    {
        if(x0==x1)
            line[++cnt]={0,/* minY or maxY */};
        else
        {
    ...
    void addSegment(X x0,X y0,X x1,X y1)
    {
        if(x0==x1)
            line[++cnt]={0,/* minY or maxY */};
        else
