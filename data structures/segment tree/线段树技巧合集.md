...
为了快速求LCA，将序列长度补齐到2的幂，且静态开点，节点 l 和 r 的LCA的深度，等于树高减 l^r 的最高位
```cpp
const uint32_t N=bit_ceil((uint32_t)5e5+3),M=bit_width(N);
namespace CatTree
{
    int a[N],ct[M][N];
    uint32_t leafSize,maxDepth;
    void init(uint32_t n)
    {
        leafSize=bit_ceil(n);
        maxDepth=bit_width(leafSize)-1;
        for(int i=n;i<leafSize;++i)//0-index
            a[i]=INF;
        for(int dep=0,len=leafSize;dep<maxDepth;++dep,len>>=1)
            for(int l=0;l<leafSize;l+=len)
            {
                int mid=l+(len>>1)-1;
                ct[dep][mid]=a[mid];
                for(int i=mid-1;i>=l;--i)
                    ct[dep][i]=min(ct[dep][i+1],a[i]);
                ct[dep][mid+1]=a[mid+1];
                for(int i=mid+2;i<l+len;++i)
                    ct[dep][i]=min(ct[dep][i-1],a[i]);
            }
    }
    int query(uint32_t l,uint32_t r)
    {
        if(l==r)
            return a[l];
        int dep=maxDepth-bit_width(l^r);
