...
1.  任意性，若$a\mid b$，则对于任意非零整数$m$，有$a\mid bm$
...
1. 分解质因数，每个质因数可以取的个数为$[0,k_i]$,n的因子个数就是$\prod\limits_{i=1}^s (1+k_i)
$  
时间复杂度为$O( s )$
...
为了求解，人为规定$gcd(a,0)=a$  
请学习$\color{powderblue}欧几里得算法\&拓展欧几里得.cpp$ $part$ $1$
...
可以用拓欧求解  
请学习$\color{powderblue}欧几里得算法\&拓展欧几里得.cpp$ $part$ $2$
...
***
# 线性同余方程组
请学习$\color{powderblue}中国剩余定理.cpp$ $part$ $1,2$

# 原根
请学习$\color{powderblue}原根.cpp$

# BSGS
请学习$\color{powderblue}bsgs.cpp$ $part$ $1,2$
