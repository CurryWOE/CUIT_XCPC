...
3、因数分解
    ...
    ii) 分解质因数，每个因数可以取的个数为[0,ai],n的因子个数就是(1 + a1) * (1 + a2) * ... * (1 + ak)。时间复杂度为O( s )

    iii) 线性筛筛选 （较复杂）
    学习线性筛.cpp part 3,并自行在不看资料的情况下实现，如果不能，说明没有理解

    d、因子和
    i)质因数分解，考虑数n，令n的因子和为s(n)，对n进行素数分解后的，假设最小素数为p，素因子p的个数为e，那么n = (p^e)n'。
    容易得知当n的因子中p的个数为0时，因子之和为s(n')。
    更加一般地，当n的因子中p的个数为k的时候，因子之和为(p^k)*s(n')，所以n的所有因子之和就可以表示成：
...
6、快速幂
    ...
    请学习快速幂.cpp part 1
    part 2涉及线性代数-矩阵乘法，理解part 1 后可以学习part 2
