...
4、最大公约数和最小公倍数
    ...
    这是一个排列组合问题，三元组{x1, y1, z1}的种类数当l == g时只有1种，否则答案就是 6(l - g)。
...
6、快速幂
    ...
    计算幂，可以用倍增加速（这里只是让你初次认识倍增，之后求解rmq问题和求解lca问题还会碰到倍增）
...
7、同余
    ...
    ≡ 是恒等于符号，表示绝对成立，无论变量取什么值，（=是条件成立，要求变量满足条件才成立）
    在模运算方程中，通常使用≡，约定俗成的习惯
