...
$O((V+E)logV)$，[洛谷模板题Prim速度第一](https://www.luogu.com.cn/record/275139435)
```cpp
const int N = 5e3 + 3;
struct Prim
{
...
namespace boruvka
{
    ...
    bool merge(int u,int v)//启发式合并维护连通块包含的点
    {
        ...
        if(connectBlock[u].size()<connectBlock[v].size())
            swap(u,v);
        connectBlock[u].reserve(connectBlock[u].size()+connectBlock[v].size());
        ranges::insert(connectBlock[u],connectBlock[u].end(),exchange(connectBlock[v], {}));
        f[v]=u;
        return true;
    }
    void init(int nn)
