...
# 标准库随机数生成器(random number generator)
...
int main()
{
    ...
    for (int i = 0; i < N; i++)
        permutation[i] = i;
    ranges::shuffle(permutation, rng);
    for (int i = 0; i < N; i++)
        cout<<permutation[i]<<"\n";
}
```
...
# XorShift随机数生成器
George Marsaglia 发明的一类伪随机数生成器，比mt19937速度快，密码学强度更弱，不过这个密码学强度够用了

随机数种子非0即可，代码里面的字面量是经过测试挑选出的
```cpp
